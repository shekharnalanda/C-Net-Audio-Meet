...
(()=>{
  ...
  [screenBtn,document.querySelector('#waitingBtn'),document.querySelector('#muteAllBtn'),document.querySelector('#unmuteAllBtn'),document.querySelector('#lockBtn')].filter(Boolean).forEach(x=>adminPanel.append(x));adminPanel.append(micLockBtn);const runHostControl=async(command,extra={},success='Control updated')=>{try{const r=await hostAct(command,extra.target);if(!r?.ok)throw Error(r?.error||'Host control failed');toast?.(success);await poll?.();return r}catch(e){toast?.(e.message||'Host control failed');return null}};const waitingControl=document.querySelector('#waitingBtn'),muteAllControl=document.querySelector('#muteAllBtn'),unmuteAllControl=document.querySelector('#unmuteAllBtn'),lockControl=document.querySelector('#lockBtn');if(waitingControl)waitingControl.onclick=()=>runHostControl('waiting_room',{},waitingControl.dataset.on==='1'?'Waiting Room बंद है।':'Waiting Room चालू है।');if(muteAllControl)muteAllControl.onclick=()=>runHostControl('mute_all',{},'सभी participants mute हैं।');if(unmuteAllControl)unmuteAllControl.onclick=()=>runHostControl('unmute_all',{},'Participants को microphone permission दी गई।');if(lockControl)lockControl.onclick=()=>runHostControl('lock',{},lockControl.textContent.includes('Unlock')?'Meeting unlocked है।':'Meeting locked है।');
