...
(()=>{
  ...
  [screenBtn,document.querySelector('#waitingBtn'),document.querySelector('#muteAllBtn'),document.querySelector('#unmuteAllBtn'),document.querySelector('#lockBtn')].filter(Boolean).forEach(x=>adminPanel.append(x));adminPanel.append(micLockBtn);const runHostControl=async(command,data={},success='Control updated')=>{try{const r=await api('control',payload({command,...data}));if(!r?.ok)throw Error(r?.error||'Host control failed');toast?.(success);await poll?.();return r}catch(e){toast?.(e.message||'Host control failed');return null}};const waitingControl=document.querySelector('#waitingBtn'),muteAllControl=document.querySelector('#muteAllBtn'),unmuteAllControl=document.querySelector('#unmuteAllBtn'),lockControl=document.querySelector('#lockBtn');if(waitingControl)waitingControl.onclick=()=>{const value=waitingControl.dataset.on!=='1';return runHostControl('waiting_room',{value},value?'Waiting Room चालू है।':'Waiting Room बंद है।')};if(muteAllControl)muteAllControl.onclick=()=>runHostControl('mute_all',{},'सभी participants mute हैं।');if(unmuteAllControl)unmuteAllControl.onclick=()=>runHostControl('unmute_all',{},'Participants को microphone permission दी गई।');if(lockControl)lockControl.onclick=()=>{const value=!lockControl.textContent.includes('Unlock');return runHostControl('lock',{value},value?'Meeting locked है।':'Meeting unlocked है।')};
  ...
  updateControls();setMediaVisual(document.querySelector('#micBtn'),true,'Microphone');setMediaVisual(cameraBtn,false,'Camera');document.documentElement.dataset.livekitAdapter='host-controls-runtime-final-ready';
})();
