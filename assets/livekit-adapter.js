...
(()=>{
  ...
  style.textContent=`
    ...
    .lk-media-btn[disabled]{opacity:.45;cursor:not-allowed}.controls>button.media-on{border-color:#28d17c!important;background:#103d2c!important}.controls>button.media-off{position:relative;border-color:#ff5b67!important;background:#4a1720!important}.controls>button.media-off::after{content:'×';position:absolute;right:2px;top:1px;width:15px;height:15px;border-radius:50%;display:grid;place-items:center;background:#e32636;color:#fff;font-size:13px!important;font-weight:900;line-height:1}
  ...
  const setMediaVisual=(button,on,label)=>{if(!button)return;button.classList.toggle('media-on',on);button.classList.toggle('media-off',!on);button.setAttribute('aria-label',`${label} ${on?'ON':'OFF'}`);button.title=`${label}: ${on?'ON':'OFF'}`};
  ...
  const adminPanel=panel('cnetAdminPanel'),reactionPanel=panel('cnetReactionPanel'),morePanel=panel('cnetMorePanel');const micLockBtn=makeButton('micLockBtn','🎙️ Participants can unmute: ON');
  ...
  [screenBtn,document.querySelector('#waitingBtn'),document.querySelector('#muteAllBtn'),document.querySelector('#lockBtn')].filter(Boolean).forEach(x=>adminPanel.append(x));document.querySelector('#unmuteAllBtn')?.remove();adminPanel.append(micLockBtn);const runHostControl=async(command,data={},success='Control updated')=>{try{const r=await api('control',payload({command,...data}));if(!r?.ok)throw Error(r?.error||'Host control failed');toast?.(success);await poll?.();return r}catch(e){toast?.(e.message||'Host control failed');return null}};const waitingControl=document.querySelector('#waitingBtn'),muteAllControl=document.querySelector('#muteAllBtn'),lockControl=document.querySelector('#lockBtn');if(waitingControl)waitingControl.onclick=()=>{const value=waitingControl.dataset.on!=='1';return runHostControl('waiting_room',{value},value?'Waiting Room चालू है।':'Waiting Room बंद है।')};if(muteAllControl)muteAllControl.onclick=()=>runHostControl('mute_all',{},'सभी participants mute हैं।');if(lockControl)lockControl.onclick=()=>{const value=!lockControl.textContent.includes('Unlock');return runHostControl('lock',{value},value?'Meeting locked है।':'Meeting unlocked है।')};
  ...
  const participantTile=(participant,self=false)=>{
    ...
    const person=adapterPeople.find(p=>p.id===identity),placeholder=document.createElement('div');placeholder.className='lk-placeholder';wrap.dataset.avatarVersion=String(person?.avatar||0);paintAvatar(placeholder,identity,participant?.name||person?.name||'Participant',person?.avatar||0);
  ...
  const updateControls=()=>{
    ...
    ['waitingBtn','muteAllBtn','lockBtn','micLockBtn'].forEach(id=>document.getElementById(id)?.classList.toggle('hidden',!host));modeBox.querySelector('#cnetSessionLabel').hidden=!creating;modeBox.querySelector('#cnetModeSelect').hidden=!creating;modeBox.querySelector('#cnetLiveTier').hidden=!creating;modeBox.querySelector('#cnetJoinVerified').hidden=creating;
  ...
  async function connectTransport(){
    ...
    try{
      ...
      stream?.getTracks?.().forEach(t=>t.stop());stream={getTracks:()=>[],getAudioTracks:()=>[{get enabled(){return !muted},set enabled(value){lkRoom?.localParticipant?.setMicrophoneEnabled(!!value)}}],getVideoTracks:()=>[]};
      if(canPublish){await lkRoom.localParticipant.setMicrophoneEnabled(prejoinMic&&!muted);muted=!prejoinMic;if(currentMode==='video'&&prejoinCamera){const publication=await lkRoom.localParticipant.setCameraEnabled(true);cameraOn=true;const localTrack=publication?.track||Array.from(lkRoom.localParticipant.cameraTrackPublications?.values?.()||[]).find(p=>p.track)?.track;attachLocalCamera(localTrack)}}setMediaVisual(document.querySelector('#micBtn'),canPublish&&!muted,'Microphone');setMediaVisual(cameraBtn,canPublish&&cameraOn,'Camera');
  ...
  updateControls();setMediaVisual(document.querySelector('#micBtn'),true,'Microphone');setMediaVisual(cameraBtn,false,'Camera');document.documentElement.dataset.livekitAdapter='safe-unmute-permission-final-ready';
})();
